...
Drop-in update:
- Keeps the existing flat `spawn_points` array.
- Adds a compiled `spawn_batches` array grouped by region_id / palette_name / method.
- Does not require any config changes.
"""
...
def build_spawn_batches(spawn_points: List[Dict]) -> List[Dict]:
    ...
    for sp in spawn_points:
        ...
        grouped.setdefault(key, []).append(sp)

    batches: List[Dict] = []
    for (region_id, palette_name, method), points in grouped.items():
        batches.append(
            {
    ...
    batches.sort(key=lambda b: b["region_id"])
    return batches
...
def generate_spawn_data(data: Dict, cfg: Dict):
    ...
    return {
        ...
        "region_summaries": region_summaries,
        "spawn_points": spawn_points,
        "spawn_batches": build_spawn_batches(spawn_points),
    }
...
def main() -> None:
    ...
    print(f"Wrote: {out_path}")
    print(f"Total spawn points: {len(out['spawn_points'])}")
    print(f"Total spawn batches: {len(out['spawn_batches'])}")
